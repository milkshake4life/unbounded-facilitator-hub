...
import { saveHeadshot } from "../lib/facilitatorsService";
...
interface HeadshotImportModalProps {
...
export function HeadshotImportModal({
  facilitators,
  onClose,
}: HeadshotImportModalProps) {
  ...
  // Facilitator ids assigned to more than one file (last upload wins).
  ...
  async function handlePick() {
    ...
    try {
      ...
      const picked = await pickFolder(token);
      if (!picked) {
        setBusy(false);
        return;
      }
      const images = await listImagesInFolder(token, picked.id);
      if (images.length === 0) {
        throw new Error("No image files were found in that folder.");
      }
      setFolder(picked);
      setMatches(buildMatches(images, facilitators));
      setStep("review");
    } catch (err) {
  ...
  /** Fetch (or reuse) the image blob + object URL for a file. */
          ...
          {!ready && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              ...
              <p className="mt-1 text-amber-700">
                This needs Firebase Storage enabled and your Google keys set. See{" "}
          ...
          {ready && step === "pick" && (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="mt-4 max-w-sm text-sm text-slate-600">
                Pick the Google Drive folder that holds the headshot photos.
                We'll match each photo to a facilitator by filename — you can
                review and fix matches before anything uploads.
              </p>
              <button
                onClick={handlePick}
                disabled={busy}
          ...
          {ready && step === "review" && (
            <div>
              ...
              <p className="mb-3 text-xs text-slate-500">
                Confirm each photo's facilitator. Unmatched photos are set to
                “Skip” — assign them if you can. Set a photo to “Skip” to leave
                it out.
              </p>
          ...
          {step === "uploading" && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="mt-3 text-sm text-slate-600">
                Uploading headshots… {progress.done} / {progress.total}
              </p>
...
function ReviewRow({
  ...
  // Lazy-load the thumbnail only when the row scrolls into view.
  useEffect(() => {
      ...
      <div className="min-w-0 flex-1">
        ...
        {match.facilitatorId ? (
          <p className="text-xs font-medium text-emerald-600">
            Matched{assigned ? `: ${assigned.firstName} ${assigned.lastName}` : ""}
            {duplicate && (
